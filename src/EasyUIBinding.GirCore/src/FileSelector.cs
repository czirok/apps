...
public class FileSelector : InputBinding<Adw.ActionRow, string>, IDisposable
{
	...
	public FileSelector(string title, string? initialPath = null, Gtk.FileFilter? fileFilter = null)
		...
		Gtk.FileFilter? fileFilter = null)
	{
		...
		_statusIcon.SetFromIconName("document-open-symbolic");
	...
	private async void OnChanged(Adw.ActionRow sender, EventArgs args)
	{
		try
		{
			var file = await _dialog.OpenAsync((Gtk.Window?)sender.GetRoot());
			if (file is not null)
			{
				_value = file.GetPath();
